...
## المرحلة 1 — إكمال الممرات المعلّقة (الأولوية القصوى) ✅ (تم — تحقّق 2026-09)

> فُحصت الثلاثة عملياً (قراءة الكود + تشغيل pytest/tsc/فحص العقد): كلها منفّذة فعلاً،
> لا مجرد مخطط. هذا القسم كان لا يزال يصف حالة سابقة — صُحِّح ليطابق الواقع (مبدأ
> "لا تفترض غياب ميزة، افحص المستودع").

### 1.1 Whisper كلمة-بكلمة في `analyst_agent.py` ✅
- منفَّذ عبر Groq API (`whisper-large-v3-turbo` افتراضياً، `OPENCODE_API_KEY`/`OPENCODE_BASE_URL`).
- استخراج mp3 16kHz mono عبر ffmpeg ثم `POST /audio/transcriptions` بـ `timestamp_granularities[]=word`.
- تدهور أنيق: غياب المفتاح أو فشل الشبكة → تحذير في `AnalystReport.warnings` بدل تعطّل المسار (لا يستخدم `_demo_words` إلا مع `--demo` صراحة).

### 1.2 الرندر الفعلي في `render_agent.py` ✅
- `build_command()` + تنفيذ فعلي عبر `subprocess.Popen` مع تتبع تقدّم (`-progress pipe:1`) ومهلة زمنية.
- يُنتج `render.json` + فيديو نهائي، ويمر عبر بوابة `validate_render`.

### 1.3 ربط المسار بواجهة Next.js ✅
- `app/api/agents/pipeline/route.ts` ينفّذ `python -m src.main` عبر `child_process.spawn` (مرحلتان: `phase=plan` ثم `phase=render` بعد تأكيد المستخدم — عارض الخطة، لا التصدير المباشر).
- `GET /api/agents/pipeline?job=<id>` يقدّم `final.mp4` الناتج.
...
| **لوحة تشخيص HTML** ✅ | عرض أدلة pipeline بملخص ملون — `/diagnostics` | تصحيح أسرع |
...
- **الاختبارات:** `src/tests/test_auto_editor.py` (15 حالة: v3، طبقات، سكون/سواد
  بثنائية مقلَّدة، `--cut`، معاينة، تكامل المخرج) — 24/24 ناجحة مع الحالية.

### التشغيل
```bash
python scripts/install-auto-editor.py      # تثبيت اختياري للثنائية (~44MB)
python -m pytest src/tests                 # 24 حالة (بلا ثنائية مضمونة أيضاً) — تحقّق 2026-09
```
...
## الجدول الزمني التقديري (تاريخي — كل بنوده مكتملة الآن، محتفظ به للسياق)

| المرحلة | المدة | المخرجات | الحالة |
|---------|-------|----------|--------|
...
| 3.x الجودة | مستمر | CI + اختبارات + لوحة تشخيص | مستمر (CI/اختبارات ✅، لوحة تشخيص لم تُبنَ بعد) |
| 4. auto-editor | — | سكون/سواد + تصدير NLE + معاينة قص | ✅ |

المراحل 1–2 و4 اكتملت فعلياً رغم أن هذا الملف لم يكن يعكس ذلك — صُحِّح 2026-09
بعد فحص الكود مباشرة (`analyst_agent.py`/`render_agent.py`/`app/api/agents/pipeline/route.ts`)
وتشغيل `pytest`/`tsc`/`check-contract.ts` كلها خضراء.

### لوحة تشخيص HTML ✅ (تم — 2026-09)
- `app/api/diagnostics/route.ts` (GET، للقراءة فقط): بلا `?job=` يُرجع قائمة كل
  مسارات `.montage_ai/pipeline/job_*` (ملخص من `manifest.json`)، مع `?job=<id>`
  يُرجع manifest + كل مُخرَجات المراحل (analyst/director/critic/audio/render).
  التحقق: `jobId` يُطابَق بـ `/^job_\d+$/` قبل أي وصول لنظام الملفات (يمنع
  path traversal — مُختبَر يدوياً 400/404). مسار غير موجود للمجلد → `{jobs: []}`
  بدل خطأ. مسار بلا `manifest.json` (تشغيل انقطع) → ملخص أدنى مبني من وجود
  ملفات المراحل بدل فشل الاستعلام بالكامل.
- `app/diagnostics/page.tsx` + `components/diagnostics/DiagnosticsDashboard.tsx`:
  قائمة مسارات (نقطة ملونة بالحالة) + لوحة تفصيل (بطاقات لكل مرحلة، تحذيرات/أخطاء
  ملونة، فيديو النتيجة النهائية قابل للتشغيل والتنزيل مباشرة إن اكتمل الرندر).
  متجاوب (قائمة تتكدس أفقياً على الشاشات الضيقة)، حالات تحميل/فراغ/خطأ صريحة،
  بلا تبعيات جديدة (fetch + React فقط، يطابق `/shorts` في النمط والتصميم).
- رابط «لوحة التشخيص» أُضيف في مودال «المسار الكامل» بجانب زر تصدير NLE
  (`components/editor/Editor.tsx`) — يفتح `/diagnostics?job=<id>` في تبويب جديد.
- **تحقّق:** `next build` نجح (المساران مسجَّلان)، اختبار وظيفي يدوي بملفات
  `.montage_ai/pipeline/` صناعية غطّى: قائمة معبأة، تفصيل كامل، قائمة فارغة،
  مسار منقطع بلا manifest، مسار غير موجود (404)، محاولة path traversal (400) —
  كلها كما هو متوقع. `pytest` 24/24 وeslint وtsc بلا أخطاء بعد التغيير.
- **الأثر:** يبقى فقط عنصر "توثيق القرارات" (`docs/decisions/`) من قائمة المرحلة 3
  الأصلية غير مبنيّ.

### إصلاح جانبي (2026-09)
أثناء التحقق، وُجد أن `AnalystAgent._analyze_sync` كان لا يزال يلتقط فقط
`NotImplementedError` حول نداءي `identify_speakers`/`track_faces` — بقايا من قبل
تنفيذهما الفعلي في المرحلة 2. كلا الدالتين الآن تتدهوران داخلياً بأناقة (pyannote/cv2
اختياريان) ولا ترفعان `NotImplementedError` أبداً، فأي خطأ داخلي غير متوقع (مثل خطأ
cv2 على إطار تالف) كان سيتسرب دون التقاط ليُسقط **المسار كاملاً** بدل تعطيل الميزة فقط
— مخالفةً لمبدأ "التدهور الأنيق إلزامي" أعلاه. وُسِّع الالتقاط إلى `Exception` ليطابق
نمط بقية الدالة (`transcribe_word_level`/`detect_motion_spans`/`detect_black_spans`).
تحقّق: 24/24 اختباراً ناجحة بعد الإصلاح.
